...
# Summary 32-02: The discriminating case set
...
|---|---|---|
| `implied_metric_is_selected_over_the_default` (32-01) | metric selection | falls to `metrics[0]`, answers on flat ticket volume |
...
Carried from 32-01 and still passing: tagging a rule-engine-passing case as `hard` fails the
...
32-03: per-tier reporting in the bench, re-measure against a model, republish. `autonomous:
false` — it spends money, and 32-01 left the bench averaging tiers into one row, which Task 1
