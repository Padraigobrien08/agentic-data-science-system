...
must_haves:
  truths:
    - "The two-part case that 28-02 had to drop as unwinnable is now winnable, and is in the hard tier."
    - "The hard tier defeats a second policy method, closing the gap phase 28 documented."
...
Purpose: 28-02 dropped a `generate_hypotheses` case because no policy could pass it. That was
...
@.planning/phases/28-agency-suite-hardening/28-agency-suite-hardening-02-SUMMARY.md
@agentic/evaluation/cases.py
@agentic/evaluation/fixtures.py
@agentic/evaluation/agency.py
@tests/agentic/test_agency_tiers.py
...
<interfaces>
The probe from 28-02 that established the limitation — the second claim never leaves `proposed`:
```
...
agentic/evaluation/cases.py
agentic/evaluation/agency.py
.planning/phases/28-agency-suite-hardening/28-agency-suite-hardening-02-SUMMARY.md</read_first>
  <behavior>
    ...
    - The case is winnable: a policy that raises both claims resolves both. Verify this before
      adding it, exactly as the 28-02 cases were verified.
...
If the tier now defeats two policy methods, say so — that was the bar phase 28 could not
reach.</action>
...
docs/agent/agency-scoreboard.md</read_first>
  <behavior>
    - Same conditions as the phase 28 run wherever unchanged — 5 trials, same model, same
...
<model> --trials 5 --tier all` under a ceiling scaled from the phase 28 observed cost ($0.35 for
...
phase 28 result below it as that one retained v1. Say explicitly whether the model handles a
...
`docs/agent/agency-scoreboard.md` retains the phase 28 measurement for comparison.
...
<output>
After completion, create `.planning/phases/29-multi-metric-investigations/29-multi-metric-investigations-03-SUMMARY.md`
</output>
