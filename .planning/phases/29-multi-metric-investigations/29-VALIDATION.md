...
`tests/agentic/test_core_tier_equivalence.py` (29-01 Task 1) pins the paths themselves — tool
...
Following the discipline established in phases 27 and 28, where two of four guards were vacuous
on first write:
...
- The new hard case (29-03): confirm it fails `FixtureAgentPolicy` *and* passes a policy that
...
- `ModelCall` persistence for agentic policy calls (27-01)
- Cached-input pricing tier — reported cost remains a slight over-estimate (27-03)
- `--max-cost-usd` truncation still unproven against a real ceiling (28-03)
