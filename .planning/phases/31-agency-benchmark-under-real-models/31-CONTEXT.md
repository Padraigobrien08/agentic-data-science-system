# Phase 31: Agency Benchmark Under Real Models - Context
...
- Watch during 31-02: if every model scores near 100%, the cases are not discriminating and
...
- The purity boundary is currently unenforced. Any convenience import from `agentic/` into
  ...
  31-02 adds the test that prevents it.
- A line-anchored grep (`^from backend`) is not sufficient to check this — the existing
  ...
  with `ast` and walk nested nodes, which is why 31-02 Task 1 specifies that explicitly.
...
- Hardening `AGENCY_CASES` if models saturate the suite — v1.6, informed by 31-02's results
