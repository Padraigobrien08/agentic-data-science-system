...
# Summary 31-01: Versioned prompts for the policy surface
...
- **`CostAwareModelPolicy.prompt_identity`** — a `prompt_id -> version` map the 31-03 scoreboard
...
31-03 records prompt versions in the scoreboard, and the bench harness runs with no DB session at
...
never pays for them. D-01, 31-VALIDATION, and 31-02 Task 1 were all corrected: the boundary test
now forbids `backend` outright and allowlists the two bridge modules for `edgar_project`/`src`.

Had this not been caught, 31-02 Task 1 would have failed on first run against existing code.
...
- **`ModelCall` persistence for agentic policy calls** — needs a decision on whether
  ...
  `agentic_investigation_execution_service` where the session already lives. Not blocking 31-02.
- `_agentic_model` still defaults to `"gpt-5.4-mini"`. Confirm it resolves before 31-03 runs a
...
31-02: observer passthrough, pure multi-trial aggregation, the bench harness, and the corrected
boundary test.
