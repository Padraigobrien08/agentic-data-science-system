...
Purpose: 28-01 proves one case can discriminate. A tier of one is a trick; a tier that only
...
@.planning/phases/28-agency-suite-hardening/28-agency-suite-hardening-01-PLAN.md
@agentic/evaluation/cases.py
@agentic/evaluation/fixtures.py
@agentic/evaluation/agency.py
...
<interfaces>
Candidate discriminators, each with the heuristic it defeats (from 28-CONTEXT `<specifics>`):
...
tests/agentic/test_agency_tiers.py</read_first>
  <behavior>
    ...
    - Each fails `FixtureAgentPolicy` on a named property — the 28-01 contract enforces this.
...
Every hard case fails `FixtureAgentPolicy` (enforced by the 28-01 contract test).
`python3 -m pytest tests/agentic -q --tb=short` passes.</acceptance_criteria>
  <verify>
    <automated>python3 -m pytest tests/agentic -q --tb=short</automated>
...
plainly that the model comparison is not yet re-run — 28-03 does that.</action>
...
<output>
After completion, create `.planning/phases/28-agency-suite-hardening/28-agency-suite-hardening-02-SUMMARY.md`
</output>
