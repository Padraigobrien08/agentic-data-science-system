...
- [x] **D1. Published OpenAPI spec artifact + versioned public API contract doc** — M _(landed 2026-08-06)_
  - `docs/api/openapi.json` (44 paths) generated by `scripts/export-openapi.py`, with `--check`
    wired into CI as an `api-contract` job. Committed rather than generated on demand so an API
    change shows up as a diff on the contract, not only inside a handler; the checker names the
    added/removed routes so a failure is actionable.
  - `docs/api/README.md` — versioning policy (additive within `/v1`), auth classes, error
    semantics including why `404` is deliberately ambiguous, and client generation.
- [x] **D2. "Add your own adapter / experiment tool" guide** — S _(landed 2026-08-06)_
  - `docs/extending.md` — a worked path through the three extension seams, complementing rather
    than restating the existing contracts.
  - The example is **executed** by `tests/agentic/test_extension_example.py`, so the guide
    cannot silently rot as the API moves.
- [x] **D3. Prove input-agnosticism with one genuinely non-financial dataset in the eval suite** — M _(landed 2026-08-06)_
  - Two non-financial fixtures (weather-station rainfall, service latency) with domain-native
    column names, plus agency cases over them. A test asserts their column names share nothing
    with the generic fixtures, so a passing case cannot be relying on names.
  - **This immediately earned its keep**: the rainfall case failed on first run, exposing that
    direction keywords were matched as *substrings* — `rainfall_mm is increasing` matched
    "fall", so the loop rejected a hypothesis its own data supported. The two call sites also
    disagreed on precedence. Fixed with a shared word-boundary parser
    (`agentic/agent/direction.py`); regressions verified to fail against the old behavior.
