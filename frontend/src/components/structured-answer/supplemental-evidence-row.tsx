    ...
    >
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between md:gap-6">
        <div className="min-w-0 space-y-1.5">
          ...
          <p className="text-[13px] leading-6 text-[var(--muted)]">{row.reason}</p>
        </div>
        {row.jump ? (
          <Link
            href={row.jump.href}
            className="shrink-0 text-[11px] font-medium text-[var(--foreground)] underline underline-offset-4 hover:text-[var(--ui-primary)]"
          >
            {row.jump.label}
          </Link>
        ) : null}
