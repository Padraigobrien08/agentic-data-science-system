 ...
 */
export function DeepDiveLayout({ includeLlmUsageAnchor, children }: Props) {
  return (
    <div className="xl:grid xl:grid-cols-[minmax(0,1fr)_12.5rem] xl:items-start xl:gap-8">
      <div className="min-w-0 space-y-4">
        <div className="xl:hidden">
          <RunTraceJumpNav variant="bar" includeLlmUsageAnchor={includeLlmUsageAnchor} />
        </div>
        {children}
      </div>
      <aside className="mt-2 hidden min-w-0 xl:sticky xl:top-4 xl:mt-0 xl:block">
        ...
        <RunTraceJumpNav variant="rail" includeLlmUsageAnchor={includeLlmUsageAnchor} />
      </aside>
    </div>
  );
}
